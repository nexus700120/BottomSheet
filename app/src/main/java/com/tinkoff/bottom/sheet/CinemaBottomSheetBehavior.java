 ...
 */
public class CinemaBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    List<WeakReference<View>> mNestedScrollingChildRefList;
    ...
    boolean mTouchingScrollingVerticalRecycler;

    /**
     * Default constructor for instantiating BottomSheetBehaviors.
     */
    public CinemaBottomSheetBehavior() {
    }
     ...
     */
    public CinemaBottomSheetBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public Parcelable onSaveInstanceState(CoordinatorLayout parent, V child) {
        return new CinemaBottomSheetBehavior.SavedState(super.onSaveInstanceState(parent, child), mState);
    }

    @Override
    public void onRestoreInstanceState(CoordinatorLayout parent, V child, Parcelable state) {
        CinemaBottomSheetBehavior.SavedState ss = (CinemaBottomSheetBehavior.SavedState) state;
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        List<View> scrollableChildren = findScrollingChildren(child);
        if (scrollableChildren.isEmpty()) {
            mNestedScrollingChildRefList = Collections.emptyList();
        } else {
            mNestedScrollingChildRefList = new ArrayList<>();
            for (View scrollableChild : scrollableChildren) {
                mNestedScrollingChildRefList.add(new WeakReference<>(scrollableChild));
            }
        }
    ...
    public boolean onInterceptTouchEvent(CoordinatorLayout parent, V child, MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                mTouchingScrollingVerticalRecycler = false;
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                if (mNestedScrollingChildRefList != null && !mNestedScrollingChildRefList.isEmpty()) {
                    for (WeakReference<View> viewWeakReference : mNestedScrollingChildRefList) {
                        View view = viewWeakReference.get();
                        if (view != null && parent.isPointInChildBounds(view, initialX, mInitialY)) {
                            mActivePointerId = event.getPointerId(event.getActionIndex());
                            mTouchingScrollingVerticalRecycler = !isVerticalRecyclerView(view);
                        }
                    }
                }
    ...
    private boolean isPointInsideChildScrollView(CoordinatorLayout parent, int x, int y) {
        ...
        for (WeakReference<View> viewWeakReference : mNestedScrollingChildRefList) {
            View view = viewWeakReference.get();
            if (view != null && parent.isPointInChildBounds(view, x, y)) {
                return true;
            }
        }
    ...
    private boolean isVerticalRecyclerView(View view) {
    ...
    public void onStopNestedScroll(CoordinatorLayout coordinatorLayout, V child, View target) {
        ...
        if (mViewDragHelper.smoothSlideViewTo(child, child.getLeft(), top)) {
            ...
            ViewCompat.postOnAnimation(child, new CinemaBottomSheetBehavior.SettleRunnable(child, targetState));
        } else {
    ...
    public boolean onNestedPreFling(CoordinatorLayout coordinatorLayout, V child, View target,
        ...
        return isOneOfChild(target) && (mState != STATE_EXPANDED ||
    ...
    @VisibleForTesting
    List<View> findScrollingChildren(View view) {
        if (ViewCompat.isNestedScrollingEnabled(view)) {
            return Collections.singletonList(view);
        }
        List<View> nestedScrollViewList = new ArrayList<>();
        if (view instanceof ViewGroup) {
            ViewGroup group = (ViewGroup) view;
            for (int i = 0, count = group.getChildCount(); i < count; i++) {
                List<View> scrollingChildList = findScrollingChildren(group.getChildAt(i));
                if (scrollingChildList != null && !scrollingChildList.isEmpty()) {
                    nestedScrollViewList.addAll(scrollingChildList);
                }
            }
        }
        return nestedScrollViewList;
    }
    ...
    void startSettlingAnimation(View child, int state) {
        ...
        if (mViewDragHelper.smoothSlideViewTo(child, child.getLeft(), top)) {
            setStateInternal(STATE_SETTLING);
            ViewCompat.postOnAnimation(child, new CinemaBottomSheetBehavior.SettleRunnable(child, state));
        } else {
    ...
    private final ViewDragHelper.Callback mDragCallback = new ViewDragHelper.Callback() {
        ...
        public boolean tryCaptureView(View child, int pointerId) {
            ...
            if (mTouchingScrollingVerticalRecycler) {
                return true;
            }
            if (mState == STATE_EXPANDED && mActivePointerId == pointerId) {
                if (mNestedScrollingChildRefList != null && !mNestedScrollingChildRefList.isEmpty()) {
                    View view = mNestedScrollingChildRefList.get(mNestedScrollingChildRefList.size() - 1).get();
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            if (mViewDragHelper.settleCapturedViewAt(releasedChild.getLeft(), top)) {
                ...
                ViewCompat.postOnAnimation(releasedChild,
                        new CinemaBottomSheetBehavior.SettleRunnable(releasedChild, targetState));
            } else {
                setStateInternal(targetState);
            }
        }
    ...
    protected static class SavedState extends AbsSavedState {
        ...
        public static final Creator<CinemaBottomSheetBehavior.SavedState> CREATOR = new ClassLoaderCreator<CinemaBottomSheetBehavior.SavedState>() {
            @Override
            public CinemaBottomSheetBehavior.SavedState createFromParcel(Parcel in, ClassLoader loader) {
                return new CinemaBottomSheetBehavior.SavedState(in, loader);
            }

            @Override
            public CinemaBottomSheetBehavior.SavedState createFromParcel(Parcel in) {
                return new CinemaBottomSheetBehavior.SavedState(in, null);
            }

            @Override
            public CinemaBottomSheetBehavior.SavedState[] newArray(int size) {
                return new CinemaBottomSheetBehavior.SavedState[size];
            }
        };
    }
